...
public class ReflectiveTreeItem extends QueriableTreeItem<Pair<Method, Object>> {
    ...
    public List<TreeItem<Pair<Method, Object>>> query() {
        ...
        return Stream.concat(getMethodsToReflect(queryableObject instanceof Class ? (Class<?>) queryableObject : queryableObject.getClass()).stream()
                        .map(m -> {
                            Object value;
                            try {
                ...
                ALIASED_METHODS.stream()
                    ...
                    .map(alias -> new TreeItem<>(new Pair<>(alias.getValue().getKey(), (Object) alias.getValue().getValue().apply(queryableObject)))))
    ...
    protected List<Method> getMethodsToReflect(Class<?> c) {
        return Arrays.stream(c.getMethods()).filter(m -> m.getParameterCount() == 0
                ...
                && BLACKLISTED_METHODS.stream().noneMatch(pair -> pair.getKey().isAssignableFrom(c) && pair.getValue().equals(m.getName()))).collect(Collectors.toList());
    }
